<template>
  <div class="table-box">
    <ProTable
      ...
      :span-col="{ xs: 2, sm: 3, md: 4, lg: 6, xl: 8 }"
    >
      <!-- 表格 header 按钮 -->
      ...
      <template #operation="scope">
        ...
        <el-button type="success" link icon="Check" v-hasPermi="['sys:product:up']" @click="openStateDialog('商品定时上架', scope.row)" v-if="scope.row.status !== 1">
          商品上架</el-button
        >
        <el-button type="danger" link icon="Bottom" v-hasPermi="['sys:product:down']" @click="openStateDialog('商品定时下架', scope.row)" v-if="scope.row.status !== 2">
          商品下架</el-button
        >
      </template>
    </ProTable>
...
// 获取 ProTable 实例，调用其获取刷新数据方法（还能获取到当前查询参数，方便导出携带参数）
const proTable = ref()
